...
*   **Why Chosen**:
    ...
    *   **Buffering**: It handles backpressure. If the Game Loop stalls, the
        ...
        leaks).
    *   **Ordering**: Guarantees that messages from *a single client* arrive
        in order.
...
*   **Why Chosen**:
    *   **Latest Value Only**: We don't care about the history of status
        changes, only the *current* state. `watch` overwrites previous values
...
would only provide the *latest* state, causing clients to miss intermediate
...
*   **Bad Pattern**: Game Loop serializes the huge JSON string and sends it.
    *   *Result*: Game Loop CPU usage spikes with high player count.
*   **Good Pattern (Our Approach)**:
    ...
    3.  Each **Net Task** serializes it to JSON *in parallel*.
